...
use, not only the one the diff touches. Then the reach of a core change: for every primitive in lunatik.h
the diff touches (\`CHECK_BASE=${a.base} bash tools/checks/blast-radius.sh lunatik.h\`), name the arm the
bug takes and the arms the fix moves; a fix that moves an arm the bug does not take is a finding, whatever
the commit body says. Then correctness: for every raise, what is held and who releases it; every get
against its put; the execution context of every path; the teardown order.
