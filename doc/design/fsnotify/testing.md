...
the verdict; and 6.8 (`6.8.0-138-generic`, arm64), where the gate does not exist and the same seven
cases pass, in the review of #852. Not yet run: a 6.14 or newer kernel, where the gate's answer is
cached per open. That is a version guard read from the source rather than executed, and it stays
unverified until someone runs the suite there.
