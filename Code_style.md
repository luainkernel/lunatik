...
Type definitions (structs, enums) use `snake_case` with a `_t` suffix for the `typedef` alias. The struct tag itself can use a `_s` suffix (e.g., [`lunatik.h`](lunatik.h:64-68)).
...
When a line exceeds the preferred limit, break it at a logical point, such as after a comma in a function argument list, after a binary operator, or before a new expression. Indent the continued line with two tabs to avoid confusion with the function body or subsequent logic.

### Function Declarations and Calls

Break after commas in parameter lists, using two tabs for indentation of the subsequent lines (e.g., [`lunatik.h`](lunatik.h:114)).
...
Break long conditional expressions after logical operators, using two tabs for indentation of the continuation (e.g., [`lunatik.h`](lunatik.h:110-111)).
