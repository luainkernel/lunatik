...
| `fsnotify` | Filesystem notification: inode, mount and superblock marks, their events, and the verdict a permission event asks for |
...
[sched_ext/scx](https://github.com/sched-ext/scx) framework and eBPF. It
includes an eBPF program which uses an eBPF map to assign queues and slices to
tasks and a Lua kernel script to set dispatch queue and slice to the tasks seen
for the first time according to a Lua [policy
table](examples/workload/workload.lua#13).
...
sudo lunatik run examples/workload/workload hardirq percpu
```
