...
local function tcp_payload(packet)
	local version_ihl = packet:getbyte(0)
	local version = version_ihl >> 4

	if version ~= 4 then
		return
	end

	local ihl = (version_ihl & 0x0f) * 4

	if packet:getbyte(9) ~= 6 then -- IPPROTO_TCP
		return
	end

	local tcp = ihl

	local dest = packet:getbyte(tcp + 2) << 8 |
		     packet:getbyte(tcp + 3)

	if dest ~= 443 then
		return
	end

	local doff = (packet:getbyte(tcp + 12) >> 4) * 4
...
local function sniclassify(ctx)
	...
	local payload = tcp_payload(data)
	if not payload then
		ctx:action(action.ACT_OK)
		return
	end

	local byte, short, str = unpacker(data, payload)
