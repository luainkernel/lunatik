...
LUNATIK_CONFIGS := \
	...
	LUNATIK_NETFILTER \
	LUNATIK_COMPLETION \
	LUNATIK_CRYPTO_SHASH \
	LUNATIK_CRYPTO_SKCIPHER \
	LUNATIK_CRYPTO_AEAD \
	LUNATIK_CRYPTO_RNG \
	LUNATIK_CRYPTO_COMP \
	LUNATIK_CPU \
...
$(foreach c,$(LUNATIK_CONFIGS),\
	$(eval CONFIG_$(c) ?= m))

LUNATIK_CONFIG_FLAGS := \
	$(foreach c,$(LUNATIK_CONFIGS),CONFIG_$(c)=$(CONFIG_$(c)))
...
defines:
	CC='$(CC)' "$(LUA)" gendefines.lua "$(KERNEL_RELEASE)" "$(INCLUDE_PATH)" "autogen" "$(LUNATIK_CONFIG_FLAGS)"
