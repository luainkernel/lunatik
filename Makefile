...
LUNATIK_MODULES := DEVICE LINUX NOTIFIER SOCKET NETLINK RCU SET TASK THREAD DATA PROBE SYSCALL XDP FIFO \
	SKB TC NETFILTER COMPLETION CRYPTO CPU HID SIGNAL BYTEORDER DARKEN BPF SCHED FSNOTIFY HANDSHAKE
...
scripts_install:
	...
	${INSTALL} -m 0644 lib/tunnel.lua ${SCRIPTS_INSTALL_PATH}/
...
scripts_uninstall:
	...
	${RM} ${SCRIPTS_INSTALL_PATH}/tunnel.lua
