#!/bin/bash
#
# SPDX-FileCopyrightText: (c) 2026 Ring Zero Desenvolvimento de Software LTDA
# SPDX-License-Identifier: MIT OR GPL-2.0-only
#
# Records what each corpus object costs the verifier, so a regression in the emitted shape is
# caught here rather than against the million-instruction limit.
#   - the "processed N insns" figure the verifier's log prints for the heaviest program of each
#     corpus is reported as a KTAP comment and asserted under a ceiling
#   - the same figure for each compiled example, read from the object `make install` put beside
#     its script rather than recompiled, so it is what a deployment loads however it was installed
# The ceiling is set from what was measured, not predicted; the figures are in
# doc/design/luaebpf/plan.md.
#
# Usage: sudo bash tests/luaebpf/budget.sh

DIR="$(dirname "$(readlink -f "$0")")"

source "$DIR/../lib.sh"
source "$DIR/common.sh"

trap cleanup EXIT

CEILING=20000

CORPORA="pass arith divzero branch forconst forvar while iter call ctx packet bounds getstring strcmp strkey strret mapget mapset struct"

luaebpf_start 2

# the escape hatch's own cost, where the module publishes the kfunc its programs call
[ -z "$(luaebpf_kfunc luaxdp bpf_luaxdp_run)" ] && CORPORA="$CORPORA callback"

worst=0
for name in $CORPORA; do
	output=$(luaebpf_compile "$name") || { comment "$output"; fail "luaebpf: $name.bpf.lua did not compile"; }
	log=$(luaebpf_verbose "$name")
	rm -rf "$LUAEBPF_PINS"; mkdir -p "$LUAEBPF_PINS"
	most=$(echo "$log" | grep -oP 'processed \K[0-9]+' | sort -n | tail -1)
	[ -n "$most" ] || { comment "$log"; fail "luaebpf: $name printed no instruction count"; }
	comment "$name: $most insns processed, worst program of the corpus"
	[ "$most" -gt "$worst" ] && worst=$most
done

# the shape a kernel with the iterators and without may_goto gets, whose cost the same ceiling
# has to cover, since the compiler picks it there and nowhere else
if [ -z "$(luaebpf_kfunc vmlinux bpf_iter_num_new)" ]; then
	output=$(luaebpf_compile while "" loadbytes,iter) \
		|| { comment "$output"; fail "luaebpf: the iterator build failed"; }
	log=$(luaebpf_verbose while)
	rm -rf "$LUAEBPF_PINS"; mkdir -p "$LUAEBPF_PINS"
	most=$(echo "$log" | grep -oP 'processed \K[0-9]+' | sort -n | tail -1)
	[ -n "$most" ] || { comment "$log"; fail "luaebpf: the iterator build printed no instruction count"; }
	comment "while under the iterator lowering: $most insns processed, worst program of the corpus"
	[ "$most" -gt "$worst" ] && worst=$most
fi

[ "$worst" -le "$CEILING" ] || fail "luaebpf: the worst corpus program cost $worst insns, over the $CEILING ceiling"
ktap_pass "luaebpf: every corpus program verifies under $CEILING processed instructions"

# what the examples the tree ships cost, from the objects beside their scripts; the classifier
# calls the kfunc, so a module publishing none leaves it with nothing to load
EXAMPLES=$LUAEBPF_FILTER
[ -z "$(luaebpf_kfunc luatc bpf_luatc_run)" ] && EXAMPLES="$EXAMPLES $LUAEBPF_CLASSIFY"

cost=0
for script in $EXAMPLES; do
	object=/lib/modules/lua/$script.bpf.o
	[ -r "$object" ] || { comment "$object is not installed"; continue; }
	log=$(luaebpf_verbose "${script##*/}" "${object%/*}")
	rm -rf "$LUAEBPF_PINS"; mkdir -p "$LUAEBPF_PINS"
	most=$(echo "$log" | grep -oP 'processed \K[0-9]+' | sort -n | tail -1)
	[ -n "$most" ] || { comment "$log"; fail "luaebpf: $script printed no instruction count"; }
	comment "$script: $most insns processed, worst program of the object"
	[ "$most" -gt "$cost" ] && cost=$most
done

if [ "$cost" -eq 0 ]; then
	ktap_skip "luaebpf: no compiled example is installed"
else
	[ "$cost" -le "$CEILING" ] \
		|| fail "luaebpf: a compiled example cost $cost insns, over the $CEILING ceiling"
	ktap_pass "luaebpf: every compiled example verifies under $CEILING processed instructions"
fi

check_dmesg
ktap_totals
[ $KTAP_FAIL -eq 0 ]

