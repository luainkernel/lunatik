...
perm_begin 8 "fsnotify/execguard"

mkdir -p -m 0700 "$SCOPE" "$OUTSIDE"
mount -t tmpfs -o size=1M,mode=0700 lunatik-execguard "$SCOPE" 2>/dev/null
mountpoint -q "$SCOPE" || perm_skip 8 "fsnotify/execguard: no tmpfs for the scope the example marks"
...
errs=$(grep -E "$KTAP_ERRORS" <<< "$output" || true)
