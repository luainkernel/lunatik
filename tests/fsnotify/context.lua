...
test("mark refuses a permission event", function()
	local watch = fsnotify.watch(report)
	local ok, err = pcall(watch.mark, watch, SCRATCH, fs.OPEN_PERM)
	watch:stop()
	assert(not ok, "mark accepted a permission event")
	assert(tostring(err):match("permission events not supported"), err)
end)
