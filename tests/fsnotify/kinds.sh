...
if ! mount -t tmpfs -o size=1M,mode=0700 lunatik-fsnotify "$MOUNT" 2>/dev/null || \
	...
	for _ in $(seq 8); do ktap_skip "fsnotify/kinds: no tmpfs to mark"; done
	ktap_totals
	exit 0
fi
...
errs=$(printf '%s\n%s\n%s\n' "$mounted" "$super" "$unmounted" | grep -E "$KTAP_ERRORS" || true)
[ -n "$errs" ] && fail "kernel error with mount and superblock marks: ${errs%%$'\n'*}"
