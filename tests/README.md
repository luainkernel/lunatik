...
- **run**: the host bytecode compiler `lunatikc`: compiled chunks (full and
  ...
  kernel; the tool refuses a syntax error, a binary input and several inputs
  with `-n` without writing anything, and writes `<input>.luac` next to the
  input by default. Skips when `lunatikc` is not installed.
