...
- **unix/address**: the AF_UNIX name `getsockname()`, `getpeername()` and
  ...
  from a peer the kernel names no address for answers with the message alone, the
  datagram one run straight after a receive from a bound peer so that a stale address
  is what an unfixed binding would return.
