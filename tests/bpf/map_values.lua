...
test("bpf.map update flag condition met succeeds", function()
	local m = map(path)
	m:delete("flg")
...
test("bpf.map key-value maps do not support push, pop, peek", function()
	local m = map(path)
	assert(not pcall(m.push, m, "val"), "expected error: hash does not support push")
	assert(not pcall(m.pop, m), "expected error: hash does not support pop")
	assert(not pcall(m.peek, m), "expected error: hash does not support peek")
	m:close()
end)
