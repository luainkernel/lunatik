...
# `lunatik run` always exits 0 and reports script errors on its output,
# so a run is only clean when it prints nothing.
run_test()
{
	local output errs
	mark_dmesg
	...
	[ -n "$errs" ] && while IFS= read -r line; do echo "# $line"; done <<< "$errs"
	return 1
}
