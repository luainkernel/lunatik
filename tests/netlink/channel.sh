...
# generic netlink multicast family ("lunatiktest") and a LOCAL_OUT netfilter
# hook that broadcasts from softirq. A userspace subscriber (built with gcc)
# joins the family's multicast group and receives the payload, proving
# kernel-to-userspace delivery from softirq context.
#
...
# generate LOCAL_OUT traffic to fire the hook (broadcast happens in softirq)
