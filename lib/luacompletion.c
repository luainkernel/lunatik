...
static int luacompletion_wait(lua_State *L)
{
	...
	lunatik_checkruntime(L, LUNATIK_FLAG_SLEEPABLE);
...
static int luacompletion_new(lua_State *L)
{
	lunatik_object_t *object = lunatik_newobject(L, &luacompletion_class, sizeof(struct completion), LUNATIK_FLAG_NONE);
