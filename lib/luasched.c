...
* `int bpf_luasched_run(const char *key, size_t key__sz, struct task_struct *task, struct task_class *cls)`
*
...
* - `task`: The task context (`struct task_struct *`).
* - `cls`: The scheduling decision (dsq and slice_ns).
*
* @function attach
