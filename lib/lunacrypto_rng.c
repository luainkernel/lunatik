 ...
 * @tparam[opt] string seed Optional seed material to mix into the RNG. If nil
 * or omitted, no explicit seed is used.
...
static int luacrypto_rng_generate(lua_State *L) {
  ...
  unsigned int num_bytes =
      (unsigned int)lunatik_checkinteger(L, 2, 1, UINT_MAX);
  ...
  lunatik_try(L, crypto_rng_generate, tfm, seed_data, (unsigned int)seed_len,
              (u8 *)buffer, num_bytes);
 ...
 * @function reset
 * @tparam[opt] string seed Optional seed material to mix into the RNG. If nil
 * or omitted, the RNG will reseed from its default sources.
...
static int luacrypto_rng_reset(lua_State *L) {
  ...
  lunatik_try(L, crypto_rng_reset, tfm, (const u8 *)seed_data,
 ...
 * @function info
 * @treturn table A table containing algorithm information, e.g., `{ driver_name
 * = "...", seedsize = num }`.
 */
 ...
 * This structure binds the C implementation (struct crypto_rng *, methods,
 ...
 * @function new
 * @tparam string algname The name of the RNG algorithm (e.g., "stdrng",
 * "drbg_nopr_ctr_aes256"). Defaults to "stdrng" if nil or omitted.
 ...
 * @within rng
 */
LUACRYPTO_NEW(rng, struct crypto_rng, crypto_alloc_rng, luacrypto_rng_class,
              luacrypto_rng_randomize);

static const luaL_Reg luacrypto_rng_lib[] = {{"new", luacrypto_rng_new},
                                             {NULL, NULL}};

LUNATIK_NEWLIB(crypto_rng, luacrypto_rng_lib, &luacrypto_rng_class, NULL);

static int __init luacrypto_rng_init(void) { return 0; }
