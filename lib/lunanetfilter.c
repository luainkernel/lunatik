...
static inline lunatik_object_t *
luanetfilter_pushskb(lua_State *L, luanetfilter_t *luanf, struct sk_buff *skb) {
...
#define luanetfilter_resetskb(data, skb, offset, header_len)                   \
  luadata_reset(data, skb, offset, skb_headlen(skb) + header_len,              \
                LUADATA_OPT_SKB)

static int luanetfilter_hook_cb(lua_State *L, luanetfilter_t *luanf,
                                struct sk_buff *skb) {
  lunatik_object_t *data;
  int ret = -1;

  if (!luanetfilter_pushcb(L, luanf) ||
      (data = luanetfilter_pushskb(L, luanf, skb)) == NULL)
    goto out;

  if (skb_mac_header_was_set(skb))
    luanetfilter_resetskb(data, skb, skb_mac_offset(skb),
...
static inline unsigned int luanetfilter_docall(luanetfilter_t *luanf,
                                               struct sk_buff *skb) {
  int ret;
...
static unsigned int luanetfilter_hook(void *priv, struct sk_buff *skb,
 ...
 *   - `hooknum` (integer): The hook number within the protocol family (e.g.,
 * `netfilter.inet_hooks.LOCAL_OUT`).
 *   - `priority` (integer): The hook priority (e.g.,
 * `netfilter.ip_priority.FILTER`).
 *   - `mark` (integer, optional): Packet mark to match. If set, the hook is
 * only called for packets with this mark.
 * @treturn userdata A handle representing the registered hook. This handle can
 * be garbage collected to unregister the hook.
 */
...
LUNATIK_NEWLIB(netfilter, luanetfilter_lib, &luanetfilter_class,
               luanetfilter_flags);

static int __init luanetfilter_init(void) { return 0; }
