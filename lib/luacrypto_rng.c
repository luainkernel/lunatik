...
LUACRYPTO_RELEASE(rng, struct crypto_rng, crypto_free_rng, );
...
static int luacrypto_rng_generate(lua_State *L) {
	...
	lunatik_try(L, crypto_rng_generate, tfm, seed_data, (unsigned int)seed_len, (u8 *)buffer, (unsigned int)num_bytes);
...
* @raise Error on failure (e.g., allocation error, crypto API error).
*/
static int luacrypto_rng_get_bytes(lua_State *L) {
	struct crypto_rng *tfm = luacrypto_rng_check(L, 1);
	lua_Integer num_bytes_l = luaL_checkinteger(L, 2);
	unsigned int num_bytes_uint = (unsigned int)num_bytes_l;
	...
	luaL_Buffer b;
	char *buffer = luaL_buffinitsize(L, &b, num_bytes_uint);

	lunatik_try(L, crypto_rng_get_bytes, tfm, (u8 *)buffer, num_bytes_uint);

	luaL_pushresultsize(&b, num_bytes_uint);
	return 1;
}
...
*/
static int luacrypto_rng_alg_info(lua_State *L) {
	struct crypto_rng *tfm = luacrypto_rng_check(L, 1);
...
static const luaL_Reg luacrypto_rng_mt[] = {
	...
	{"get_bytes", luacrypto_rng_get_bytes},
	{"alg_info", luacrypto_rng_alg_info},
...
*/
LUACRYPTO_NEW(rng, struct crypto_rng, crypto_alloc_rng, luacrypto_rng_class, tfm,
