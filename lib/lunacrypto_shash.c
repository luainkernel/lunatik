...
LUACRYPTO_RELEASER(shash, struct shash_desc, lunatik_free,
                   luacrypto_shash_release_tfm);
 ...
 * Updates the hash state with the given data, then finalizes and returns the
 * digest. `init()` must have been called prior to calling `finup()`.
 * @function finup
 ...
 * Must be called after `init()` and any `update()` calls if part of a
 ...
 * This overwrites the current hash state and allows resuming a hash
 * calculation. The imported state must be compatible with the current hash
 * algorithm.
 * @function import
 * @tparam string state The previously exported hash state (binary string).
 * @raise Error on failure or if the provided state length is incorrect for the
 * algorithm.
 */
 ...
 * This structure binds the C implementation (luacrypto_shash_tfm_t, methods,
 ...
 * @function new
 * @tparam string algname The name of the hash algorithm (e.g., "sha256",
 * "hmac(sha256)").
 * @treturn crypto_shash The new SHASH object.
 * @raise Error if the TFM object or kernel descriptor cannot be
 ...
 * @within shash
 */
static struct shash_desc *luacrypto_shash_new_sdesc(lua_State *L,
...
LUACRYPTO_NEW(shash, struct crypto_shash, crypto_alloc_shash,
              luacrypto_shash_class, luacrypto_shash_new_sdesc);

static const luaL_Reg luacrypto_shash_lib[] = {{"new", luacrypto_shash_new},
                                               {NULL, NULL}};

LUNATIK_NEWLIB(crypto_shash, luacrypto_shash_lib, &luacrypto_shash_class, NULL);

static int __init luacrypto_shash_init(void) { return 0; }
