/*
 * SPDX-FileCopyrightText: (c) 2024-2026 Mohammad Shehar Yaar Tausif
 * <sheharyaar48@gmail.com> SPDX-License-Identifier: MIT OR GPL-2.0-only
 */
...
#include "lunadata.h"
#include "lunanetfilter.h"
#include "lunarcu.h"
#include "lunaxtable.h"
 ...
 * It encapsulates the kernel structures (`struct xt_match` or `struct
 * xt_target`) and the Lunatik runtime information necessary to invoke the Lua
 * callbacks. The primary way to interact with this object from Lua is to let it
 * be garbage collected, which will unregister the extension from the kernel.
...
static int luaxtable_docall(lua_State *L, luaxtable_t *xtable,
                            luaxtable_info_t *info, const char *op, int nargs,
                            int nret) {
...
static inline lunatik_object_t *luaxtable_getskb(lua_State *L,
                                                 luaxtable_t *xtable) {
...
static int luaxtable_pushparams(lua_State *L, const struct xt_action_param *par,
                                luaxtable_t *xtable, struct sk_buff *skb,
                                uint8_t opt) {
  ...
  luadata_reset(data, skb->data, 0, skb->len, opt);
...
#define luaxtable_call(L, op, xtable, skb, par, info, opt)                     \
  ((luaxtable_pushparams(L, par, xtable, skb, opt) == -1) ||                   \
   (luaxtable_docall(L, xtable, info, op, 2, 1) == -1))

static int luaxtable_domatch(lua_State *L, luaxtable_t *xtable,
                             const struct sk_buff *skb,
                             struct xt_action_param *par, int fallback) {
  if (luaxtable_call(L, "match", xtable, (struct sk_buff *)skb, par,
                     (luaxtable_info_t *)par->matchinfo,
                     LUADATA_OPT_READONLY) != 0)
    return fallback;
...
static int luaxtable_dotarget(lua_State *L, luaxtable_t *xtable,
                              struct sk_buff *skb,
                              const struct xt_action_param *par, int fallback) {
  if (luaxtable_call(L, "target", xtable, skb, par,
                     (luaxtable_info_t *)par->targinfo, LUADATA_OPT_NONE) != 0)
    return fallback;
...
#define LUAXTABLE_HOOK_CB(hook, huk, U, V, T)                                  \
  static T luaxtable_##hook(U skb, V par) {                                    \
    ...
    lunatik_run(xtable->runtime, luaxtable_do##hook, ret, xtable, skb, par,    \
                luaxtable_hooks.hook##_fallback);                              \
    return ret;                                                                \
  }
...
#define LUAXTABLE_CHECKER_CB(hook, hk, huk, HOOK)                              \
  static int luaxtable_##hook##_check(const struct xt_##hk##chk_param *par) {  \
    int ret;                                                                   \
    luaxtable_t *xtable;                                                       \
    lunatik_object_t *obj = luarcu_gettable(                                   \
        luaxtable_hooks.hook, par->hook->name, XT_EXTENSION_MAXNAMELEN);       \
    if (obj == NULL) {                                                         \
    ...
    info->data = xtable;                                                       \
                                                                               \
    lunatik_run(xtable->runtime, luaxtable_docall, ret, xtable, info,          \
                "checkentry", 0, 1);                                           \
    return ret != 0 ? -EINVAL : 0;                                             \
  }

#define LUAXTABLE_DESTROYER_CB(hook, hk, huk, HOOK)                            \
  static void luaxtable_##hook##_destroy(                                      \
      const struct xt_##hk##dtor_param *par) {                                 \
    int ret;                                                                   \
    luaxtable_info_t *info = (luaxtable_info_t *)par->huk##info;               \
...
LUAXTABLE_HOOK_CB(match, match, const struct sk_buff *,
                  struct xt_action_param *, bool);
LUAXTABLE_HOOK_CB(target, targ, struct sk_buff *,
                  const struct xt_action_param *, unsigned int);
...
* @function match
* @tparam table opts A table containing the configuration for the match
extension.
*   It **must** include the following fields:
*
*   - `name` (string): The unique name for this match extension (e.g.,
"myluamatch"). This name is used in `iptables -m <name>`.
*   - `revision` (integer): The revision number of this match extension.
*   - `family` (netfilter.family): The protocol family this match applies to
(e.g., `netfilter.family.INET`).
*   - `proto` (socket.ipproto, optional, default: 0): The specific IP protocol
this match applies to (e.g., `socket.ipproto.TCP`). Use 0 for any protocol.
*   - `hooks` (integer): A bitmask indicating the Netfilter hooks where this
match can be used (e.g., `1 << netfilter.inet_hooks.LOCAL_OUT`). Multiple hooks
can be OR'd. Note: `netfilter.netdev_hooks` are not available for legacy
Xtables.
*   - `match` (function): The Lua function to be called to evaluate if a packet
matches.
*     Its signature is `function(skb, par, userargs) -> boolean`.
*
*     - `skb` (data): A read-only `data` object representing the packet's socket
buffer.
*     - `par` (table): A read-only table containing parameters related to the
packet and hook:
*       - `hotdrop` (boolean): `true` if an earlier rule already marked the
packet for unconditional drop.
*       - `thoff` (integer): Offset to the transport header within the `skb`.
*       - `fragoff` (integer): Fragment offset (0 if not a fragment or is the
first fragment).
*       - `hooknum` (integer): The Netfilter hook number (e.g.,
`NF_INET_LOCAL_OUT`).
*     - `userargs` (string): A string containing any arguments passed to this
match from the `iptables` rule command line.
*
*     The function should return `true` if the packet matches, `false`
otherwise.
*   - `checkentry` (function): A Lua function called when an `iptables` rule
using this match is added or modified.
*     Its signature is `function(userargs)`.

*     - `userargs` (string): The arguments string from the `iptables` rule.

*     This function should validate the `userargs`. If validation fails, it
should call `error()`.
*   - `destroy` (function): A Lua function called when an `iptables` rule using
this match is deleted.
*     Its signature is `function(userargs)`.
*
*     - `userargs` (string): The arguments string from the `iptables` rule.
*
*     This function can be used for cleanup.
* @treturn xtable_extension A userdata object representing the registered match
extension.
...
* @raise Error if registration fails (e.g., name conflict, invalid parameters,
...
*   -- To use in iptables: iptables -A INPUT -m myluamatch --somearg "hello" -j
ACCEPT
*/
 ...
 * @function target
 * @tparam table opts A table containing the configuration for the target
 * extension. It **must** include the following fields:
 *
 *   - `name` (string): The unique name for this target extension (e.g.,
 * "MYLUATARGET"). This name is used in `iptables -j <NAME>`.
 *   - `revision` (integer): The revision number of this target extension.
 *   - `family` (netfilter.family): The protocol family this target applies to
 * (e.g., `netfilter.family.INET`).
 *   - `proto` (socket.ipproto, optional, default: 0): The specific IP protocol
 * this target applies to (e.g., `socket.ipproto.UDP`). Use 0 for any protocol.
 *   - `hooks` (integer): A bitmask indicating the Netfilter hooks where this
 * target can be used.
 *   - `target` (function): The Lua function to be called to process a packet.
 *     Its signature is `function(skb, par, userargs) ->
 * netfilter.action_verdict`.
 *     - `skb` (data): A read-write `data` object representing the packet's
 * socket buffer. Modifications to this `skb` can alter the packet.
 *     - `par` (table): A read-only table containing parameters related to the
 * packet and hook (same structure as for `xtable.match`).
 *     - `userargs` (string): A string containing any arguments passed to this
 * target from the `iptables` rule command line.
 *
 *     The function should return an integer verdict, typically one of the
 * constants from the `netfilter.action` table (e.g., `netfilter.action.DROP`,
 * `netfilter.action.ACCEPT`).
 *   - `checkentry` (function): A Lua function called when an `iptables` rule
 * using this target is added or modified. Its signature is
 * `function(userargs)`. (See `xtable.match` for details).
 *   - `destroy` (function): A Lua function called when an `iptable`s rule using
 * this target is deleted. Its signature is `function(userargs)`. (See
 * `xtable.match` for details).
 * @treturn xtable_extension A userdata object representing the registered
 * target extension.
 ...
 *   -- To use in iptables: iptables -A FORWARD -j MYLUATARGET --someoption
 * "value"
 */
static const luaL_Reg luaxtable_mt[] = {{"__gc", lunatik_deleteobject},
                                        {NULL, NULL}};
...
static inline lunatik_object_t *luaxtable_new(lua_State *L, int idx, int hook) {
  ...
  lunatik_object_t *object =
      lunatik_newobject(L, &luaxtable_class, sizeof(luaxtable_t));
...
static inline void luaxtable_register(lua_State *L, int idx,
                                      luaxtable_t *xtable,
                                      lunatik_object_t *object) {
...
#define LUAXTABLE_NEWHOOK(hook, HOOK)                                          \
  static int luaxtable_new##hook(lua_State *L) {                               \
    ...
    hook->destroy = luaxtable_##hook##_destroy;                                \
                                                                               \
    if (luarcu_settable(luaxtable_hooks.hook, hook->name,                      \
                        XT_EXTENSION_MAXNAMELEN, object) != 0)                 \
...
static const luaL_Reg luaxtable_lib[] = {{"match", luaxtable_newmatch},
...
#define luaxtable_inithook(hook)                                               \
  (luaxtable_hooks.hook = luarcu_newtable(LUARCU_DEFAULT_SIZE, false))
