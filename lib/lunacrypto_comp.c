...
#define LUACRYPTO_COMP_OPERATION(name)                                         \
  static int luacrypto_comp_##name(lua_State *L) {                             \
    ...
    unsigned int max_len =                                                     \
        (unsigned int)lunatik_checkinteger(L, 3, 1, UINT_MAX);                 \
                                                                               \
    luaL_Buffer B;                                                             \
    u8 *output_buf = luaL_buffinitsize(L, &B, max_len);                        \
                                                                               \
    lunatik_try(L, crypto_comp_##name, tfm, data, (unsigned int)datalen,       \
                output_buf, &max_len);                                         \
 ...
 * provide a size slightly larger than the input data (e.g., input size + a
 * small fixed overhead or percentage).
 * @function compress
 * @tparam string data The data to compress.
 * @tparam integer max_output_len The maximum possible size of the compressed
 * data.
 ...
 * @tparam integer max_output_len The maximum possible size of the decompressed
 * data.
 * @treturn string The decompressed data.
 * @raise Error on failure (e.g., allocation error, crypto API error, input data
 * corrupted, `max_output_len` too small).
 */
 ...
 * @function new
 * @tparam string algname The name of the compression algorithm (e.g., "lz4",
 * "deflate").
...
static const luaL_Reg luacrypto_comp_lib[] = {{"new", luacrypto_comp_new},
                                              {NULL, NULL}};

LUNATIK_NEWLIB(crypto_comp, luacrypto_comp_lib, &luacrypto_comp_class, NULL);
#endif

static int __init luacrypto_comp_init(void) { return 0; }
