	...
	luafsnotify_event_t *event)
{
	...
	if (lunatik_getregistry(L, object) != LUA_TUSERDATA) {
		pr_err_ratelimited("couldn't find event\n");
		return NULL;
	}
...
static inline void luafsnotify_bind(lua_State *L, int ix, lunatik_object_t *object, luafsnotify_mark_t *mark)
{
	object->private = mark;
	mark->object = object;
	lunatik_register(L, ix, mark); /* the handle lives as long as the mark: find() hands it back */
}
...
{
	luafsnotify_t *watch = (luafsnotify_t *)private;
...
/* before 6.10 the API takes a connector, and a mount's is behind real_mount(), private to fs/mount.h */
...
{
	static const char *const kinds[] = {"inode", "mount", "sb", NULL};
	static const unsigned int types[] = {FSNOTIFY_OBJ_TYPE_INODE, FSNOTIFY_OBJ_TYPE_VFSMOUNT, FSNOTIFY_OBJ_TYPE_SB};
	unsigned int type = types[luaL_checkoption(L, ix, "inode", kinds)];

#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 10, 0))
	luaL_argcheck(L, type != FSNOTIFY_OBJ_TYPE_VFSMOUNT, ix, "\"mount\" needs kernel 6.10 or later");
#endif
	return type;
}
...
static int luafsnotify_mark(lua_State *L)
{
	...
	__u32 mask = luafsnotify_checkmask(L, 3);
...
static int luafsnotify_mask(lua_State *L)
{
	...
	if (!lua_isnoneornil(L, 2)) {
		__u32 mask = luafsnotify_checkmask(L, 2);
...
* Carried by the directory entry events, the old directory for `RENAME`, and by
* an event on an object whose parent directory watches its children, whether or
* not `name` is.
* @function dir
...
{
	luafsnotify_checkevent(L, 1); /* only a live event answers, and it lives in the accessing task */

	lua_pushinteger(L, (lua_Integer)task_pid_nr(current));
	return 1;
}
