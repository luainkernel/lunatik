...
* `int bpf_luatc_run(char *key, size_t key__sz, struct __sk_buff *skb, void *arg, size_t arg__sz)`
*
...
* - `tc_ctx`: The TC metadata context (`struct __sk_buff *`).
...
*    and control the TC verdict via `ctx:action()`.
*
