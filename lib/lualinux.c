...
* @treturn[opt] string The symbolic name of the error if found, otherwise `nil`.
* @usage
* local name = linux.errname(2)
* if name then print("Error name:", name) end -- "ENOENT"
*/
static int lualinux_errname(lua_State *L)
{
	int e = (int)luaL_checkinteger(L, 1);
	const char *n = lunatik_errname(e);
	if (n && *n != '\0' && strcmp(n, "unknown") != 0)
		lua_pushstring(L, n);
	else
		lua_pushinteger(L, e);
	return 1;
