...
static const struct hid_device_id *luahid_setidtable(lua_State *L, int idx) {
  ...
  struct hid_device_id *user_table =
      lunatik_checkalloc(L, sizeof(struct hid_device_id) * (len + 1));
...
static inline int luahid_pcall(lua_State *L, lua_CFunction op,
                               luahid_ctx_t *ctx) {
...
static inline void luahid_pushhdev(lua_State *L,
                                   const struct hid_device *hdev) {
...
static inline void luahid_pushreport(lua_State *L,
                                     const struct hid_report *report) {
  lua_newtable(L);
...
static inline lunatik_object_t *luahid_pushdata(lua_State *L,
                                                luahid_ctx_t *ctx) {
...
static void luahid_op(lua_State *L, luahid_ctx_t *ctx, int nargs) {
  ...
  lunatik_optcfunction(L, -1, ctx->cb,
                       lunatik_nop); /* stack: args, hid, hid.cb */
...
static luahid_rdesc_t luahid_report_fixup(struct hid_device *hdev, __u8 *rdesc,
                                          unsigned int *rsize) {
  luahid_t *hid = luahid_gethid(hdev);
...
static int luahid_raw_event(struct hid_device *hdev, struct hid_report *report,
                            u8 *data, int size) {
  luahid_t *hid = luahid_gethid(hdev);
 ...
 * This function creates a new HID driver object from a Lua table and registers
 * it with the kernel. The Lua table must contain the following fields:
 *
 * - `name`: The name of the driver (string).
 * - `id_table`: A table of device IDs that this driver can match against
 * (lua_table).
...
static int luahid_register(lua_State *L) {
  ...
  lunatik_object_t *object =
      lunatik_newobject(L, &luahid_class, sizeof(luahid_t));
  ...
  luaL_argcheck(L, (driver->id_table = luahid_setidtable(L, -1)) != NULL, 1,
                "invaild id_table");
...
static int __init luahid_init(void) { return 0; }
