/*
 * SPDX-FileCopyrightText: (c) 2023-2026 Ring Zero Desenvolvimento de Software
...
#define LUADATA_TOPTR(d)                                                       \
  (((d)->opt & LUADATA_OPT_SKB ? (LUADATA_TOSKB(d)->data) : (d)->ptr) +        \
   (d)->offset)
...
/***
 * Bounds-checked pointer calculation. Returns pointer on success, raises Lua
 * error on failure.
 * @param L Lua state
 ...
 */
static inline void *luadata_checkbounds(lua_State *L, int ix, luadata_t *data,
                                        lua_Integer offset,
                                        lua_Integer length) {
...
#define luadata_checkwritable(L, data)                                         \
  luaL_argcheck((L), !((data)->opt & LUADATA_OPT_READONLY), 1, "read only")
...
#define LUADATA_NEWINT_GETTER(T)                                               \
  static int luadata_get##T(lua_State *L) {                                    \
    ...
    T##_t value =                                                              \
        *(T##_t *)luadata_checkbounds(L, 2, data, offset, sizeof(T##_t));      \
 ...
 * @function getstring
 * @tparam integer offset Byte offset from the start of the data block
 * (0-indexed).
 * @tparam[opt] integer length Number of bytes to read. If omitted, reads all
 * bytes from `offset` to the end of the data block.
 ...
 * @function setstring
 * @tparam integer offset Byte offset from the start of the data block
 * (0-indexed) where writing will begin.
 * @tparam string s The string to write into the data object.
 * @raise Error if the write operation (offset + length of string) goes out of
 * bounds, or if the data object is read-only.
 */
 ...
 * @within data
 */
static const luaL_Reg luadata_lib[] = {{"new", luadata_lnew}, {NULL, NULL}};
...
static const luaL_Reg luadata_mt[] = {
     ...
     * @tparam integer offset Byte offset from the start of the data block
     * (0-indexed).
     * @treturn integer The unsigned 8-bit integer value (0-255) at the
     * specified offset.
     * @raise Error if offset is out of bounds.
     */
     ...
     * Assumes host byte order. For specific byte orders, use `linux.be16toh`
     * etc. on the result.
     * @function getint16
     * @tparam integer offset Byte offset from the start of the data block
     * (0-indexed).
     ...
     * Assumes host byte order. For specific byte orders, use `linux.htobe16`
     * etc. on the value before setting.
     * @function setint16
     * @tparam integer offset Byte offset from the start of the data block
     * (0-indexed).
     ...
     * @tparam integer offset Byte offset from the start of the data block
     * (0-indexed).
     * @treturn integer The unsigned 16-bit integer value at the specified
     * offset.
     * @raise Error if offset is out of bounds.
     */
     ...
     * @tparam integer offset Byte offset from the start of the data block
     * (0-indexed).
     * @treturn integer The unsigned 32-bit integer value at the specified
     * offset.
     * @raise Error if offset is out of bounds.
     */
...
static inline void luadata_set(luadata_t *data, void *ptr, ptrdiff_t offset,
                               size_t size, uint8_t opt) {
  data->ptr = ptr;
  data->offset = offset;
  data->size = size;
  data->opt = opt;
...
static int luadata_lnew(lua_State *L) {
  ...
  lunatik_object_t *object =
      lunatik_newobject(L, &luadata_class, sizeof(luadata_t));
...
static inline lunatik_object_t *luadata_create(void *ptr, size_t size,
                                               bool sleep, uint8_t opt) {
  lunatik_object_t *object =
      lunatik_createobject(&luadata_class, sizeof(luadata_t), sleep);
...
lunatik_object_t *luadata_new(lua_State *L) {
  lunatik_object_t *data =
      lunatik_checknull(L, luadata_create(NULL, 0, false, LUADATA_OPT_NONE));
  lunatik_cloneobject(L, data);
  return data;
}
EXPORT_SYMBOL(luadata_new);

int luadata_reset(lunatik_object_t *object, void *ptr, ptrdiff_t offset,
                  size_t size, uint8_t opt) {
  luadata_t *data;
...
static int __init luadata_init(void) { return 0; }
