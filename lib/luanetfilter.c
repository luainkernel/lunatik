...
static inline bool luanetfilter_checkconn(struct sk_buff *skb, u32 ctmark)
{
	...
	if (unlikely(!ct)) {
		pr_err("conntrack is not available");
		return false;
	}
	return READ_ONCE(ct->mark) == ctmark;
}
...
#endif
	if (!lua_isnil(L, -2))
		skb->mark = (u32)lua_tointeger(L, -2);
	return lua_tointeger(L, -3);
}

#define LUANETFILTER_ISENABLED(luanf, OPT)	(luanf->opt & LUANETFILTER_OPT_##OPT)
...
static inline unsigned int luanetfilter_docall(luanetfilter_t *luanf, struct sk_buff *skb)
{
	...
	if (likely(LUANETFILTER_ISENABLED(luanf, MARK) && luanf->mark != skb->mark))
		goto out;

#if IS_ENABLED(CONFIG_NF_CONNTRACK)
	if (likely(LUANETFILTER_ISENABLED(luanf, CTMARK) && !luanetfilter_checkconn(skb, luanf->ctmark)))
		goto out;
#endif
