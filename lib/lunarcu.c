/*
 * SPDX-FileCopyrightText: (c) 2023-2025 Ring Zero Desenvolvimento de Software
 * LTDA SPDX-License-Identifier: MIT OR GPL-2.0-only
 */

/***
 * Read-Copy-Update (RCU) synchronized hash table.
 * This library provides a Lua-accessible hash table that uses RCU
 * (Read-Copy-Update) for synchronization within the Linux kernel. RCU allows
 * for very fast, lockless read operations, while write operations (updates and
 * deletions) are synchronized to ensure data consistency. This makes it highly
 * suitable for scenarios where read operations significantly outnumber write
 * operations and high concurrency is required.
 *
...
#define luarcu_sizeoftable(size)                                               \
  (sizeof(luarcu_table_t) + sizeof(struct hlist_head) * (size))

/* size is always a power of 2; thus `size - 1` turns on every valid bit */
#define luarcu_mask(table) ((table)->size - 1)
#define luarcu_hash(table, key, keylen)                                        \
  (lunatik_hash((key), (keylen), (table)->seed) & luarcu_mask(table))
#define luarcu_seed() get_random_u32()

#define luarcu_entry(ptr, pos)                                                 \
  hlist_entry_safe(rcu_dereference_raw(ptr), typeof(*(pos)), hlist)
#define luarcu_foreach(table, bucket, n, pos)                                  \
  for (bucket = 0, pos = NULL; pos == NULL && bucket < (table)->size;          \
       bucket++)                                                               \
    for (pos = luarcu_entry(hlist_first_rcu(&(table)->hlist[bucket]), pos);    \
         pos && ({                                                             \
           n = luarcu_entry(hlist_next_rcu(&(pos)->hlist), pos);               \
           1;                                                                  \
         });                                                                   \
         pos = n)
...
static inline luarcu_entry_t *luarcu_lookup(luarcu_table_t *table,
                                            unsigned int index, const char *key,
                                            size_t keylen) {
  luarcu_entry_t *entry;

  hlist_for_each_entry_rcu(entry, table->hlist + index,
                           hlist) if (strncmp(entry->key, key, keylen) ==
                                      0) return entry;
  return NULL;
}

static luarcu_entry_t *luarcu_newentry(const char *key, size_t keylen,
                                       lunatik_object_t *object) {
  luarcu_entry_t *entry;

  if (keylen >= LUARCU_MAXKEY ||
      (entry = kmalloc(struct_size(entry, key, keylen + 1), GFP_ATOMIC)) ==
          NULL)
    return NULL;
...
lunatik_object_t *luarcu_gettable(lunatik_object_t *table, const char *key,
                                  size_t keylen) {
...
int luarcu_settable(lunatik_object_t *table, const char *key, size_t keylen,
                    lunatik_object_t *object) {
  int ret = 0;
 ...
 * `rcu_table[key]`. Read operations are RCU-protected and lockless.
 * @function __index
 * @tparam rcu_table self The RCU table instance.
 * @tparam string key The key to look up in the table.
 * @treturn lunatik_object The Lunatik object associated with the key, or `nil`
 * if the key is not found. A new reference to the object is returned.
 * @usage
 ...
 * `rcu_table[key] = value`. Write operations are synchronized.
 * @function __newindex
 * @tparam rcu_table self The RCU table instance.
 * @tparam string key The key to set or update.
 * @tparam lunatik_object|nil value The Lunatik object to associate with the
 * key. If `nil`, the key-value pair is removed from the table.
...
static inline int luarcu_map_call(lua_State *L, int cb, const char *key,
                                  lunatik_object_t *value) {
 ...
 * @function map
 * @tparam rcu_table self The RCU table instance.
 * @tparam function callback A Lua function that will be called for each entry
 * in the table. The callback receives two arguments:
 *
...
static const struct luaL_Reg luarcu_lib[] = {
    {"table", luarcu_table}, {"map", luarcu_map}, {NULL, NULL}};

static const struct luaL_Reg luarcu_mt[] = {{"__newindex", luarcu_newindex},
...
lunatik_object_t *luarcu_newtable(size_t size, bool sleep) {
  ...
  if ((object = lunatik_createobject(&luarcu_class, luarcu_sizeoftable(size),
                                     sleep)) != NULL)
 ...
 * @function table
 * @tparam[opt=1024] integer size Specifies the initial number of hash buckets
 * (internal slots) for the table. This is **not** a hard limit on the number of
 * entries the table can store. The provided `size` will be rounded up to the
 * nearest power of two. Choosing an appropriate `size` involves a trade-off
 * between memory usage and performance:
 *
 *   - more buckets (larger `size`): Consumes more memory for the table
 * structure itself, even if many buckets remain empty. However, it reduces the
 * probability of hash collisions, which can significantly speed up operations
 * (lookups, insertions, deletions), especially when storing a large number of
 * entries.
 *   - fewer buckets (smaller `size`): Uses less memory for the table's internal
 * array. However, if the number of entries is high relative to the number of
 * buckets, it increases the chance of hash collisions. This means more entries
 * might end up in the same bucket, forming longer linked lists that need to be
 * traversed, thereby slowing down operations.
 *
 *   **Guidance**: For optimal performance, aim for a `size` that is roughly in
 * the order of, or somewhat larger than, the maximum number of entries you
 * anticipate storing. This helps maintain a low average number of entries per
 * bucket (a low "load factor," ideally close to 1). The table can hold more
 * entries than its `size` (number of buckets), but performance will degrade as
 * the load factor increases. The default is a general-purpose starting point
 *   suitable for many common use cases.
 * @treturn rcu_table A new RCU table object, or raises an error if memory
 * allocation fails.
 * @usage
 *   local my_rcu_table = rcu.table() -- Default: 1024 buckets, good for
 * moderate entries. local small_table = rcu.table(128) -- 128 buckets, for
 * fewer expected entries. local large_table = rcu.table(8192) -- 8192 buckets,
 * for many expected entries.
 * @within rcu
 */
static int luarcu_table(lua_State *L) {
  size_t size = roundup_pow_of_two(luaL_optinteger(L, 1, LUARCU_DEFAULT_SIZE));
  lunatik_object_t *object =
      lunatik_newobject(L, &luarcu_class, luarcu_sizeoftable(size));
...
static int __init luarcu_init(void) { return 0; }
