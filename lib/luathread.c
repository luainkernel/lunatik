...
static const lunatik_class_t luathread_class = {
	...
	.flags = LUNATIK_FLAG_SLEEPABLE | LUNATIK_FLAG_SHARABLE,
};

#define luathread_new(L)	(lunatik_newobject((L), &luathread_class, sizeof(luathread_t), LUNATIK_FLAG_SHARABLE))
...
static int luathread_run(lua_State *L)
{
	...
	luaL_argcheck(L, runtime->flags & LUNATIK_FLAG_SLEEPABLE, 1, "cannot use non-sleepable runtime in this context");
