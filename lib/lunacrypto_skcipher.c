 ...
 * which can then be used for encryption and decryption with various block
 ...
 * @raise Error if setting the key fails (e.g., invalid key length for the
 * algorithm).
 */
 ...
 * Data processed by encrypt/decrypt should typically be a multiple of this
 * size, depending on the cipher mode.
 * @function blocksize
...
static inline void
luacrypto_skcipher_newrequest(lua_State *L,
  ...
  luaL_argcheck(L, request->iv_len == crypto_skcipher_ivsize(tfm), 2,
                "incorrect IV length");
...
static inline void
luacrypto_skcipher_setrequest(luacrypto_skcipher_request_t *request,
                              char *buffer) {
 ...
 * for most modes. Plaintext length should be appropriate for the cipher mode
 * (e.g., multiple of blocksize).
 * @function encrypt
 * @tparam string iv The Initialization Vector. Its length must match
 * `ivsize()`.
 * @tparam string plaintext The data to encrypt.
 * @treturn string The ciphertext.
 * @raise Error on encryption failure, incorrect IV length, or allocation
 * issues.
 */
 ...
 * @raise Error on decryption failure, incorrect IV length, or allocation
 * issues.
 */
 ...
 * This structure binds the C implementation (luacrypto_skcipher_t, methods,
 * release function) to the Lua object system managed by Lunatik.
 */
 ...
 * @function new
 * @tparam string algname The name of the skcipher algorithm (e.g., "cbc(aes)",
 * "ctr(aes)").
 * @treturn skcipher The new SKCIPHER TFM object.
 * @raise Error if the TFM object or kernel request cannot be
 ...
 * @within skcipher
 */
LUACRYPTO_NEW(skcipher, struct crypto_skcipher, crypto_alloc_skcipher,
              luacrypto_skcipher_class, NULL);

static const luaL_Reg luacrypto_skcipher_lib[] = {
    {"new", luacrypto_skcipher_new}, {NULL, NULL}};

LUNATIK_NEWLIB(crypto_skcipher, luacrypto_skcipher_lib,
               &luacrypto_skcipher_class, NULL);

static int __init luacrypto_skcipher_init(void) { return 0; }
