...
static int luasocket_send(lua_State *L)
{
    ...
    if (lua_type(L, 2) == LUA_TSTRING)
        ...
        luadata_t *data = luadata_check(L, 2);
        lua_Integer offset = luaL_checkinteger(L, 3);
        len = (size_t)luaL_checkinteger(L, 4);
        luadata_checkbounds(L, 3, data->size, offset, len);
        vec.iov_base = (void *)(data->ptr + offset);
        ixadrr = 5;
    }
	vec.iov_len = len;
...
* @tparam[opt=false] boolean from
*   If `true`, the function also returns the sender's address and port (for
*   `AF_INET`). This is typically used with connectionless sockets (`SOCK_DGRAM`).
*
* @treturn string|integer
*   - String form: the received message as a Lua string of bytes.
*   - `data` form: the number of bytes written into the `data` buffer.
* @treturn[opt] integer|string addr
*   If `from` is true, the sender's address.
*   - For `AF_INET`: An integer IPv4 address (convert with `net.ntoa()`).
*   - For other families: A packed string representing the sender's address.
* @treturn[opt] integer port
*   If `from` is true and the family is `AF_INET`, the sender's port number.
