...
static size_t luasocket_checkaddr(lua_State *L, struct socket *socket, struct sockaddr_storage *addr, int ix)
{
	...
	if (addr->ss_family == AF_INET) {
		...
		lua_Integer proto = luaL_checkinteger(L, ix);
		luaL_argcheck(L, proto >= 0 && proto <= U16_MAX, ix, "protocol out of bounds");
		addr_ll->sll_protocol = htons((u16)proto);
		addr_ll->sll_ifindex  = (int)luaL_checkinteger(L, ix + 1);
