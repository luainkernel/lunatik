/*
 * SPDX-FileCopyrightText: (c) 2024-2026 Ring Zero Desenvolvimento de Software
...
static inline lunatik_object_t *luaxdp_pushdata(lua_State *L, int upvalue,
...
static int luaxdp_handler(lua_State *L, struct xdp_buff *ctx, void *arg,
                          size_t arg__sz) {
  int action = -1;
  int status;
...
static inline int luaxdp_checkruntimes(void) {
  ...
  if (luaxdp_runtimes == NULL && (luaxdp_runtimes = luarcu_gettable(
                                      lunatik_env, key, sizeof(key))) == NULL)
    return -1;
  return 0;
}

__bpf_kfunc int bpf_luaxdp_run(char *key, size_t key__sz,
                               struct xdp_md *xdp_ctx, void *arg,
                               size_t arg__sz) {
...
/***
 * Unregisters the Lua callback function associated with the current Lunatik
 * runtime. After calling this, `bpf_luaxdp_run` calls targeting this runtime
 * will no longer invoke a Lua function (they will likely return an error or
 * default action).
 * @function detach
 * @treturn nil
 * @usage
 ...
 *
 * The `bpf_luaxdp_run` kfunc is called from an eBPF program with the following
 * signature: `int bpf_luaxdp_run(char *key, size_t key_sz, struct xdp_md
 * *xdp_ctx, void *arg, size_t arg_sz)`
 *
 * - `key`: A string identifying the Lunatik runtime (e.g., the script name like
 * "examples/filter/sni"). This key is used to look up the runtime in Lunatik's
 * internal table of active runtimes.
 ...
 *
 * @function attach
 * @tparam function callback The Lua function to be called. This function
 * receives two arguments:
 *
 * 1. `buffer` (data): A `data` object representing the network packet buffer
 * (`xdp_md`). The `data` object points to `xdp_ctx->data` and its size is
 * `xdp_ctx->data_end - xdp_ctx->data`.
 * 2. `argument` (data): A `data` object representing the `arg` passed from the
 * eBPF program. Its size is `arg_sz`.
 *
 *   The callback function should return an integer verdict, typically one of
 * the values from the `xdp.action` table (e.g., `xdp.action.PASS`,
 * `xdp.action.DROP`).
 * @treturn nil
 * @raise Error if the current runtime is sleepable or if internal setup fails.
 * @usage
 *   -- Lua script (e.g., "my_xdp_handler.lua" which is run via `lunatik run
 * my_xdp_handler.lua`) local xdp = require("xdp")
 *
 ...
 * These constants define the possible return values from an XDP program (and
 * thus from the Lua callback attached via `xdp.attach`) to indicate how the
 * packet should be handled. (Constants from `<uapi/linux/bpf.h>`)
 * @table action
 *   @tfield integer ABORTED Indicates an error; packet is dropped.
 * (XDP_ABORTED)
 *   @tfield integer DROP Drop the packet silently. (XDP_DROP)
 *   @tfield integer PASS Pass the packet to the normal network stack.
 * (XDP_PASS)
 *   @tfield integer TX Transmit the packet back out the same interface it
 * arrived on. (XDP_TX)
 *   @tfield integer REDIRECT Redirect the packet to another interface or BPF
 * map. (XDP_REDIRECT)
 * @within xdp
 */
static const lunatik_reg_t luaxdp_action[] = {
    {"ABORTED", XDP_ABORTED}, {"DROP", XDP_DROP},         {"PASS", XDP_PASS},
    {"TX", XDP_TX},           {"REDIRECT", XDP_REDIRECT}, {NULL, 0}};

static const lunatik_namespace_t luaxdp_flags[] = {{"action", luaxdp_action},
                                                   {NULL, NULL}};
