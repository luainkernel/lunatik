...
lunatik_object_t *luarcu_newtable(size_t size, bool sleep)
{
	...
	if ((object = lunatik_createobject(&luarcu_class, luarcu_sizeoftable(size), sleep, true))!= NULL)
...
static int luarcu_table(lua_State *L)
{
	...
	lunatik_object_t *object = lunatik_newobject(L, &luarcu_class, luarcu_sizeoftable(size), true);
