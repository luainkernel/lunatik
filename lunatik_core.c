...
{
	lunatik_object_t * __percpu *runtimes = (lunatik_object_t * __percpu *)private;
	int cpu;
...
static int lunatik_stoppercpu(lua_State *L)
{
	...
	lunatik_object_t * __percpu *runtimes = lunatik_percpuruntimes(object);
	int cpu;
...
static int lunatik_percpu(lua_State *L)
{
	...
	lunatik_object_t *object = lunatik_newobject(L, &lunatik_percpu_class, 0, LUNATIK_OPT_NONE);
	...
	for_each_possible_cpu(cpu) {
		if (lunatik_newruntime(per_cpu_ptr(runtimes, cpu), L, script, opt, cpu) != 0) {
			object->private = NULL;
			lunatik_releasepercpu(runtimes); /* stop the instances now, not on collection */
			lua_error(L);
		}
	}
	return 1;
